...
function DynamoDBExportCSV(awsAccessKeyId, awsSecretAccessKey, awsRegion) {
    ...
    function getResumption(table, segment, compressed, s3Bucket, s3Path, done) {
        if (s3Bucket) {
            // Not supported at this time
            return setImmediate(done);
        }

        var baseName = table + "-" + segment + "-";
        var suffix = ".csv"
        if (compressed) {
            suffix += ".gz";
        }
        ...
        for (lastCount = 0; lastCount < maxIterations; ++lastCount) {
            var fileName = table + '/' + baseName + lastCount + suffix;
            if (!fs.existsSync(fileName)) {
                break;
            }
        }
        if (lastCount == 0 || lastCount == maxIterations) {
            // No files found
            return setImmediate(done);
        }

        var fileName = table + '/' + baseName + (lastCount - 1) + suffix;
        ...
        var csvStream = csv.parse().on('data', function(data) {
            ...
            done(null, lastCount, firstRow[0], lastRow[0]);
        }).on('error', function(err) {
            done(err);
        });
        if (compressed) {
            readableStream.pipe(new zlib.Gunzip()).pipe(csvStream);
        } else {
    ...
    function writeTableToCsv(table, columns, totalSegments, segment, compressed, filesize, s3Bucket, s3Path, callback) {
        ...
        getResumption(table, segment, compressed, s3Bucket, s3Path, function(err, resumeCount, resumeKeyName, resumeKeyValue) {
            if (!err && resumeCount) {
                fileCount = resumeCount;
                query.ExclusiveStartKey = {};
                query.ExclusiveStartKey[resumeKeyName] = {"S": resumeKeyValue};
            }
            async.doWhilst(
                function (done) {
